...
- Added `Output.get_vcd()` and the `VcdMode` class which return the calculated VCD spectrum from the ORCA output file (#XXX).
- Added a tutorial notebook on the calculation of a VCD spectrum (#XXX).
