...
- `Output.get_energies()` now also reports the corrections that ORCA adds on top of the total energy of the electronic structure method: the dispersion correction (`VdW`) and the geometrical counterpoise correction (`gCP`) (#283).
...
- Added `EnergyType`, which enumerates and documents the known keys of the dictionary returned by `Output.get_energies()` (#283).
